...
def run_fid(real_dir: Path, generated_dir: Path, device: str) -> float:
    output = run_cmd(
        [
            sys.executable,
            "-m",
...
def run_ssim(
    ...
    for cond_path in list_images(conditioning_dir):
        gen_path = generated_dir / cond_path.name
...
def run_cmmd(
    ...
    command = command_template.format(ref_dir=ref_dir, eval_dir=eval_dir)
    proc = subprocess.run(command, shell=True, capture_output=True, text=True)
    if proc.returncode != 0:
        raise RuntimeError(
            f"CMMD command failed ({proc.returncode}): {command}\n"
            f"STDOUT:\n{proc.stdout}\nSTDERR:\n{proc.stderr}"
        )
    combined = f"{proc.stdout}\n{proc.stderr}"
    return parse_last_float(combined)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Shell command template to compute CMMD. "
            "Use placeholders {ref_dir} and {eval_dir}. "
            "Example: \"python main.py --ref_dir {ref_dir} --eval_dir {eval_dir}\""
        ),
    )
    ...
    for cond in conditions:
        ...
        fid = run_fid(cond.real_dir, cond.generated_dir, args.device)
        print(f"FID: {fid:.6f}")
        ...
        cmmd = run_cmmd(cond.real_dir, cond.generated_dir, args.cmmd_cmd_template)
